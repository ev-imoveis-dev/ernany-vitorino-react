...
function obterTelefoneCorretor(property, config) {
  return formatarWhatsApp(
    property?.corretor?.whatsapp
    || property?.corretor?.celular
...
function PropertyDetailContent({ propertyId }) {
  ...
  useEffect(() => {
    ...
    getImovelById(propertyId)
      ...
      .catch(() => {
        if (!cancelled) setErro('Imovel nao encontrado.')
      })
      .finally(() => {
  ...
  const metaTitulo = property
    ? `${property.nome} - ${Number(property.valor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} | Ernany Vitorino Imoveis`
    : null
  ...
  function handleWhatsAppClick() {
    ...
    const numero = obterTelefoneCorretor(property, config)
    if (!numero) {
      setWhatsappError('Nao ha um numero de WhatsApp disponivel no momento.')
      return
    }

    const referenciaImovel = property?.referencia ? ` Referencia: ${property.referencia}.` : ''
    const texto = encodeURIComponent(
      `Ola! Me chamo ${formData.nome.trim()}, gostaria de falar sobre o imovel ${property?.nome || 'selecionado'}.${referenciaImovel}`,
    )
  ...
  if (erro || !property) {
    return <div className="pt-40 text-center text-red-400 text-lg">{erro || 'Imovel nao encontrado.'}</div>
  }
  ...
  const listaImagens = Array.isArray(imagens) && imagens.length > 0 ? imagens : (imagem ? [imagem] : [])
      ...
      <div className="container mx-auto px-4 md:px-6 py-12">
          ...
          <div className="lg:col-span-2 space-y-12">
            <div>
              ...
              <div className="text-3xl font-bold text-secondary">
                ...
                {tipo === 'aluguel' && (
                  <span className="text-base text-gray-400 font-normal ml-2">/mes</span>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 p-8 bg-light rounded-2xl">
              {[
                ...
                { icon: Square, label: 'Area Total', value: tamanho ? `${tamanho}m2` : '-' },
            ...
            <div className="sticky top-32 space-y-6">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-4 mb-8">
                    <div>
                      ...
                      <span className="text-secondary text-sm font-medium">Corretor Responsavel</span>
                    </div>
                  </div>
              ...
              <div className="bg-light p-6 rounded-2xl border border-gray-100 flex items-start gap-4">
                ...
                <p className="text-xs text-gray-500 leading-relaxed">
                  Os valores e disponibilidades dos imoveis podem sofrer alteracoes sem aviso previo.
                  Entre em contato para confirmar as informacoes.
                </p>
              </div>
            </div>
          </aside>
        ...
        {similares.length > 0 && (
          <section className="mt-24 pt-24 border-t">
            <h2 className="text-3xl font-serif text-primary mb-12">Imoveis Similares</h2>
...
export default function PropertyDetail() {
  const { id } = useParams()
  return <PropertyDetailContent key={id} propertyId={id} />
}
